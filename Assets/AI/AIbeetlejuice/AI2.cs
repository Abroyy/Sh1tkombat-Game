...
public class AI2 : MonoBehaviour
{
    ...
    void MoveTowardsPlayer()
    {
        ...
        if(step==0){
            anim.SetBool("Enemywalk2",false);
        }
        else{
            anim.SetBool("Enemywalk2",true);
        }
    ...
    void EfirstSkillShoot()
    {
        if (distance > 1.2 || distance < -1.2)
        {
            if (bulletattackTimer <= 0)
            {
                ...
                muzikbutton1.Play();
                anim.SetTrigger("Enemyfirstskill2");
    ...
    void ESEcondSkillShoot()
    {
        if (a <= 0)
        {
            if (distance < 1 && distance > -1)
            {
                ...
                anim.SetTrigger("Enemysecondskill2");
            }
        }
        a -= Time.deltaTime;
    }
