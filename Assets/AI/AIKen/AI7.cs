...
public class AI7 : MonoBehaviour
{
    ...
    void MoveTowardsPlayer()
    {
        ...
        if(step==0){
            anim.SetBool("Enemywalk7",false);
        }
        else{
            anim.SetBool("Enemywalk7",true);
        }
    ...
    void EfirstSkillShoot()
    {
        if (distance > 1.2 || distance < -1.2)
        {
            if (bulletattackTimer <= 0)
            {
                ...
                muzikbutton1.Play();
                anim.SetTrigger("Enemyfirstskill7");
    ...
    void ESEcondSkillShoot()
    {
        if (a <= 0)
        {
            if (distance < 1 && distance > -1)
            {
                ...
                attackDamage = 20f;
                AttackPlayer();
                anim.SetTrigger("Enemysecondskill7");
