...
public class AI9 : MonoBehaviour
{
    ...
    float distance;
    public float enemyscalex = 0.24f;
    Vector2 scaler;
    ...
    void MoveTowardsPlayer()
    {
        ...
        if(step==0){
            anim.SetBool("Enemywalk9",false);
        }
        else{
            anim.SetBool("Enemywalk9",true);
        }
    ...
    void EfirstSkillShoot()
    {
        if (distance > 1.2 || distance < -1.2)
        {
            if (bulletattackTimer <= 0)
            {
                ...
                muzikbutton1.Play();
                anim.SetTrigger("Enemyfirstskill9");
    ...
    void ESEcondSkillShoot()
    {
        if (a <= 0)
        {
            if (distance < 1 && distance > -1)
            {
                ...
                attackDamage = 25f;
                AttackPlayer();
                anim.SetTrigger("Enemysecondskill9");
