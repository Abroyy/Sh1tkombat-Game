...
public class AI5 : MonoBehaviour
{
    ...
    float distance;
    public float enemyscalex = 0.15f;
    Vector2 scaler;
    ...
    void MoveTowardsPlayer()
    {
        ...
        if(step==0){
            anim.SetBool("Enemywalk5",false);
        }
        else{
            anim.SetBool("Enemywalk5",true);
        }
    ...
    void EfirstSkillShoot()
    {
        if (distance > 1.2 || distance < -1.2)
        {
            if (bulletattackTimer <= 0)
            {
                ...
                muzikbutton1.Play();
                anim.SetTrigger("Enemyfirstskill5");
    ...
    void ESEcondSkillShoot()
    {
        if (a <= 0)
        {
            if (distance < 1 && distance > -1)
            {
                ...
                attackDamage = 20f;
                AttackPlayer();
                anim.SetTrigger("Enemysecondskill5");
