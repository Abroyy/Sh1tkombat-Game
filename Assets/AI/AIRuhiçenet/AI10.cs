...
public class AI10 : MonoBehaviour
{
    ...
    float distance;
    public float enemyscalex = 0.27f;
    Vector2 scaler;
    ...
    public void TakeDamage(float damage)
    {
        if (attackTimer2 <= 0)
        {
            ...
            anim.SetTrigger("Enemydefence10");
            damage -= 4;
    ...
    void MoveTowardsPlayer()
    {
        ...
        if(step==0){
            anim.SetBool("Enemywalk10",false);
        }
        else{
            anim.SetBool("Enemywalk10",true);
        }
    ...
    { 
        ...
        if(distance < 1 && distance > -1)
        {
            if (attackTimer <= 0)
            {
            anim.SetTrigger("Enemyfirstskill10");
            muzikbutton1.Play();
            attackDamage = 20;
            AttackPlayer();
    ...
    void ESEcondSkillShoot()
    {
            if(a<=0)
            {
                anim.SetTrigger("Enemysecondskill10");
                muzikbutton2.Play();
                currentHealth += 25f;
