...
public class AI13 : MonoBehaviour
{
    ...
    public float secondskillcooldawn = 14f;
    ...
    public void TakeDamage(float damage)
    {
        if (attackTimer2 <= 0)
        {
            ...
            anim.SetTrigger("Enemydefence13");
            damage -= 4;
    ...
    void MoveTowardsPlayer()
    {
        ...
        if(step==0){
            anim.SetBool("Enemywalk13",false);
        }
        else{
            anim.SetBool("Enemywalk13",true);
        }
    ...
    void EfirstSkillShoot()
    {
        if (distance > 1.2 || distance < -1.2)
        {
            if (bulletattackTimer <= 0)
            {
                ...
                muzikbutton1.Play();
                anim.SetTrigger("Enemyfirstskill13");
    ...
    void ESEcondSkillShoot()
    {
        if (a <= 0)
        {
            if (distance < 1 && distance > -1)
            {
                ...
                attackDamage = 20f;
                AttackPlayer();
                anim.SetTrigger("Enemysecondskill13");
