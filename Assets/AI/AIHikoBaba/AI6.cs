...
public class AI6 : MonoBehaviour
{
    ...
    public float bulletSpeed2 = 7f;
    public float bulletfiretime = 3f;
    public float secondskillcooldawn = 5f;
    private Rigidbody2D rb2d;
    public float bulletattackTimer = 3f;
    private float attackTimer = 5f;
    private float timer = 2;
    private float timer2 = 2;
    private float bulletattackTimer2 = 2f;
    float distance;
    public float enemyscalex = 0.5f;
    Vector2 scaler;
    ...
    void MoveTowardsPlayer()
    {
        ...
        if(step==0){
            anim.SetBool("Enemywalk6",false);
        }
        else{
            anim.SetBool("Enemywalk6",true);
        }
    ...
    void EfirstSkillShoot()
    {
        if (distance > 1.2 || distance < -1.2)
        {
            if (bulletattackTimer <= 0)
            {
                ...
                muzikbutton1.Play();
                anim.SetTrigger("Enemyfirstskill6");
    ...
    {
        if (distance > 2.7 || distance < -2.7)
        {
            if (bulletattackTimer2 <= 0)
            {
                GameObject mermi = Instantiate(bullet2, firePoint.position, firePoint.rotation);
                muzikbutton2.Play();
                anim.SetTrigger("Enemysecondskill6");
                if (transform.localScale.x >= 0f)
                {
                    Rigidbody2D rb = mermi.GetComponent<Rigidbody2D>();
                    rb.velocity = Vector3.right * bulletSpeed2;
