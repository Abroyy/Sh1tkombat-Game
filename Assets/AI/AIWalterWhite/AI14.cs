...
public class AI14 : MonoBehaviour
{
    ...
    public void TakeDamage(float damage)
    {
        if (attackTimer2 <= 0)
        {
            ...
            anim.SetTrigger("Enemydefence14");
            damage -= 4;
    ...
    void MoveTowardsPlayer()
    {
        ...
        if(step==0){
            anim.SetBool("Enemywalk14",false);
        }
        else{
            anim.SetBool("Enemywalk14",true);
        }
    ...
    void EfirstSkillShoot()
    {
        if (distance > 1.2 || distance < -1.2)
        {
            if (bulletattackTimer <= 0)
            {
                ...
                muzikbutton1.Play();
                anim.SetTrigger("Enemyfirstskill14");
