...
public class AI1 : MonoBehaviour
{
    ...
    public float bulletSpeed2 = 10f;
    public float bulletfiretime = 3f;
    public float secondskillcooldawn = 13f;
    ...
    void MoveTowardsPlayer()
    {
        ...
        if(step==0){
            anim.SetBool("Enemywalk1",false);
        }
        else{
            anim.SetBool("Enemywalk1",true);
        }
    }
    ...
    void EfirstSkillShoot()
    {
        if (distance > 1.2 || distance < -1.2)
        {
            ...
            {
                anim.SetTrigger("Enemyfirstskill1");
                muzikbutton1.Play();
    ...
    public void ESEcondSkillShoot()
    {
        if (a <= 0)
        {
            if (distance < -6 || distance > 6 || distance < 1 && distance > -1)
            {
                {
                    GameObject mermi = Instantiate(bullet2, firePoint.position, firePoint.rotation);
                    muzikbutton2.Play();

                    anim.SetTrigger("Enemysecondskill1");
                    if (scaler.x <= 0)
                    {
                        Rigidbody2D rb = mermi.GetComponent<Rigidbody2D>();
                        Vector2 launchDirection = new Vector2(-Mathf.Cos(45), Mathf.Sin(45));
                        rb.velocity = launchDirection * bulletSpeed2;
